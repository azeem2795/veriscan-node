...
import { NODE_ENV, ORIGIN, ADMIN, CLIENT } from '@config';

// Common Middleware
export default function middleware(app: Application): void {
  app.use(
    cors({ origin: NODE_ENV === 'prod' ? [ORIGIN, ADMIN, CLIENT] : true, credentials: true }),
  ); // CORS (Cross Origin Policy) to restrict unknown requests
