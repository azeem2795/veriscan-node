...
import { NODE_ENV, ORIGIN, ADMIN } from '@config';

// Common Middleware
export default function middleware(app: Application): void {
  app.use(cors({ origin: NODE_ENV === 'prod' ? [ORIGIN, ADMIN] : true, credentials: true })); // CORS (Cross Origin Policy) to restrict unknown requests
